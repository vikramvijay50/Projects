				...
				if (firstName == "" || lastName == "" || originAirport == "" || destinationAirport == "" || departureDate == "" || seatNum == "") {
					...
					if (!result.next()) {
						...
						if(numSeats <= 0){
							...
							updateWaiting.setString(3, firstName);
							updateWaiting.setString(4, lastName);
							updateWaiting.setBoolean(5, false);
						...
						if(seatClass.equals("Economy")){
							...
							insertReservation.setString(2, session.getAttribute("username").toString());
						...
						PreparedStatement updateAlert = connection.prepareStatement("DELETE FROM waiting_list WHERE username=? AND Fnum=?");
						updateAlert.setString(1, session.getAttribute("username").toString());
						updateAlert.setInt(2, flightNum);
						updateAlert.executeUpdate();
							...
							if (!result2.next()) {
								...
								PreparedStatement updateAlert2 = connection.prepareStatement("DELETE FROM waiting_list WHERE username=? AND Fnum=?");
								updateAlert2.setString(1, session.getAttribute("username").toString());
								updateAlert2.setInt(2, flightNum2);
								updateAlert2.executeUpdate();
